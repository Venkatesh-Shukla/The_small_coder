...
%% M2: Model Fitting Milestone Report
%% The Small Coder: Democratizing Competitive Programming via SLM Fine-Tuning
...
\begin{frontmatter}
    ...
    \title{The Small Coder: Democratizing Competitive Programming Reasoning via Multi-Stage Fine-Tuning of a 1.5B Parameter Small Language Model}

    \author[inst1]{Venkatesh Shukla}

    \address[inst1]{NIT Kurukshetra, Computer Science and Engineering}

    \begin{abstract}
        This report presents Milestone 2 (M2) results for \textit{The Small Coder} project — a multi-stage fine-tuning pipeline applied to DeepSeek-R1-Distill-Qwen-1.5B targeting competitive programming (CP) problem solving. Standard large-scale reasoning models require enterprise-grade hardware, making them inaccessible for edge deployment. We investigate whether a 1.5B Small Language Model (SLM), through strategic curriculum learning, teacher-student logic distillation, and structured evaluation, can achieve meaningful functional correctness on CP benchmarks. Our Train-Validate-Test pipeline uses 12,000 samples from the rStar-Coder dataset (Stage 1) and 2,000 teacher-distilled ``High-Density Logical Blueprints'' generated via Gemma 3 27B (Stage 2). Evaluation against hidden test cases in a Python sandbox on 200 problems reveals a jump from 0\% to 8.0\% Pass@1, alongside significant gains in reasoning density (Think Density: $<$1\% $\rightarrow$ $\sim$56\%) and stable format adherence (94\%). Failure analysis on 184 failed cases shows 69\% Wrong Answer (WA), 25\% Runtime Error (RE), 3.3\% Time Limit Exceeded (TLE), and 2.7\% format failures — confirming the model now generates runnable code and requires targeted logical refinement. The project repository is available at \url{https://github.com/Venkatesh-Shukla/The_small_coder}.
    \end{abstract}

    \begin{keyword}
        Small Language Models \sep Competitive Programming \sep Fine-Tuning \sep Curriculum Learning \sep Knowledge Distillation \sep Reasoning \sep Pass@1
    \end{keyword}
...
The current landscape of Generative AI is dominated by Large Language Models (LLMs) with hundreds of billions of parameters. While highly capable, such models present two significant barriers to widespread adoption:

\begin{enumerate}[label=(\roman*)]
    \item \textbf{Hardware Accessibility:} Models of 70B+ parameters require enterprise-grade A100/H100 GPUs, precluding local PC deployment or privacy-focused edge computing.
    \item \textbf{Compute Constraints:} Training a reasoning model from scratch is computationally prohibitive for individual researchers.
\end{enumerate}

\textit{The Small Coder} directly addresses these barriers. Our objective is to democratize high-level reasoning by fine-tuning \texttt{DeepSeek-R1-Distill-Qwen-1.5B} — a model runnable on a standard consumer PC — to achieve competitive-programming-level logical reasoning. Figure~\ref{fig:pipeline} illustrates the end-to-end multi-stage pipeline.
...
This M2 report documents our model fitting approach, train-validate-test split methodology, evaluation metrics, and interpretability of results.
...
\textbf{Knowledge Distillation.} Hinton et al.~\cite{hinton2015distilling} established the teacher-student paradigm for transferring knowledge from large models to smaller ones. Our Stage 2 pipeline operationalizes this by using Gemma 3 27B as a teacher to generate concise reasoning traces for the 1.5B student.

\textbf{Curriculum Learning.} Bengio et al.~\cite{bengio2009curriculum} demonstrated that presenting training samples in order of increasing complexity improves model convergence. We apply this by sorting problems by token length during Stage 1 fine-tuning.

\textbf{Reasoning in SLMs.} DeepSeek-R1~\cite{deepseek2024r1} showed that distilled smaller models can exhibit non-trivial reasoning. We extend this line of research to the domain of competitive programming, where functional correctness is the primary metric.

%%-----------------------------------------------------------
\section{Dataset \& Train-Validate-Test Split}
\label{sec:data}
%%-----------------------------------------------------------
...
\begin{itemize}
    \item \textbf{rStar-Coder Dataset (Stage 1):} 12,000 competitive programming samples covering problem statements, reasoning traces, and solution code, sourced from the rStar-Coder benchmark~\cite{rstar2024}. Stratified as 30\% Easy, 40\% Medium, 30\% Hard.
...
All data was split at the problem level to prevent data leakage. Table~\ref{tab:split} summarizes the split ratios applied per stage.

\begin{table}[H]
    \centering
    \caption{Train-Validate-Test Split Configuration}
    \label{tab:split}
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Stage} & \textbf{Dataset} & \textbf{Total} & \textbf{Train} & \textbf{Validate} & \textbf{Test} \\
        \midrule
        Stage 1        & rStar-Coder      & 12,000         & 9,600 (80\%)   & 1,200 (10\%)      & 1,200 (10\%)  \\
        Stage 2        & Gemma Distilled  & 2,000          & 1,600 (80\%)   & 200 (10\%)        & 200 (10\%)    \\
        Final Eval     & Hidden Sandbox   & 200            & ---            & ---               & 200 (100\%)   \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Split Strategy:} Problems were stratified by difficulty tier (estimated by token length) before splitting, ensuring each partition contains a representative distribution of Easy, Medium, and Hard problems.
...
\begin{itemize}
    ...
    \item Its distilled initialization provides a stronger reasoning prior than a vanilla 1.5B model.
...
We began by building ``muscle memory'' using 9,600 training samples from the rStar-Coder dataset. Problems were sorted by ascending token length, operationalizing curriculum learning~\cite{bengio2009curriculum}: the model first stabilizes its understanding of syntax and output format before encountering complex algorithmic logic.

\textbf{Hyperparameters:} LoRA rank 16, learning rate 2e-4, batch size 8 (2 per device $\times$ 4 gradient accumulation), 2 epochs, $\sim$2,250 total steps. Total trainable parameters: 18.46M of 1.795B (1.03\%).
...
\begin{figure}[H]
    ...
    \caption{Training and validation loss curves for Stage 1 (rStar-Coder curriculum, 2,250 steps) and Stage 2 (Gemma-distilled blueprints). Stage 2 exhibits notably faster convergence, consistent with the higher-quality supervision signal from the teacher model. Stage 1 hyperparameters: LoRA rank 16, lr=2e-4, batch=8, 2 epochs, 2,250 steps, 18.46M trainable parameters.}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.12\linewidth} p{0.38\linewidth} p{0.38\linewidth}}
        ...
        \midrule
        \textbf{Think Density} & $\sim$3\%                                                                                & $\sim$61\%                                                                                    \\
        \bottomrule
    \end{tabular}
\end{table}
...
\textbf{Stage 5 — RAG Integration (External Library):} Retrieval-Augmented Generation over a database of human-verified algorithm templates (Segment Trees, DSU, FFT). RAG provides the ``syntax skeleton,'' freeing the model's 1.5B parameters to focus on problem-specific logic. The 25\% RE rate (unhandled exceptions, recursion errors) motivates this component.
...
\begin{thebibliography}{9}
    ...
    \bibitem{rstar2024}
    X. Qi et al.,
    \textit{rStar-Math: Small LLMs Can Master Math Reasoning with Self-Evolved Deep Thinking},
